...
## 2026-09-18 — A model per role, and a judge that can supply the scorecard's numbers
**Decided:** `src/brain/roles.ts` grows from two roles to six — `persona`, `expert`, `scores`, `brief`, `flow`, `personagen` — plus `expert:<id>` as a map key for one named expert. `--model-for <role>=<model>` (repeatable) and `LEAKDOWN_MODELS` fill a `ModelsByRole`; `getBrain` resolves `expert:<id>` → `expert` → `--model` → the CLI's own default, so a role nobody pins is untouched and a run with no flag is byte-identical to before. Parsing and validation sit in `parseModelMap` (`src/brain/picker.ts`), called from `parseCommon` — before a browser or a mailbox exists (invariant 9). Tool policy is unchanged and stays two buckets, now via `toolPolicy()`, which is invariant 17. A tiered session records its map as `modelFor` in `meta.json` and is filed under the persona's model, not `--model`. Separately, `Judge` gains an optional `scoreDimensions()`; when a judge implements it the Conversion Scorecard takes its five numbers from there and the model writes only the notes. The judge is asked **before** the model, and its scores go into the prompt as settled.
**Why:** `--model` pinned every call to one model, so the session loop — one call per step, and the only prose a reader sees — ran on whatever the cheapest JSON call could tolerate. Measured on this repo's own runs: 24 untiered haiku sessions average $0.2273 over 6.5 steps; the same pipeline with `persona=sonnet` costs $0.3042 over 2.3 steps. Roughly 3× per step, which is the point — it is a decision now rather than an accident of one flag. The scorecard is a separate problem: a prose model re-reading one journey scores it 6 one day and 8 the next, and that drift is larger than the gap most `--variant` arms show, so two arms could not be ranked on it. A judge answers on one scale and the numbers subtract. All five dimensions or none: a card with two judged numbers and three written ones is on two scales at once and reads as one. The ordering was paid for once: asking the model first and swapping its numbers afterwards printed a note written to justify a 6 beside the judge's 1, and one row of a live card read as a contradiction. Asking the judge first and handing the model its numbers fixed it — same call count.
**Rejected:** a role per expert (eleven enum entries, and `roles.ts` would need editing every time an expert is registered — `expert:<id>` falling back to `expert` gives the same control and a new expert inherits the panel's model); a judge on the `--stop` check (it runs every step, before thinking, and is deliberately free — a call there costs one per step to catch a miss nobody has measured); asking the judge for notes as well as numbers (it returns probabilities and no prose, which is the same boundary already recorded above); defaulting a new role to the expert tool policy (that widens what site text can reach — invariant 5 by another route, so `toolPolicy()` makes every role an explicit choice).
**Files:** `src/brain/roles.ts`, `src/brain/index.ts`, `src/brain/picker.ts`, `src/brain/picker.test.ts` (new), `src/brain/judge.ts`, `src/brain/adapters/claude.ts`, `src/brain/adapters/opencode.ts`, `src/experts/types.ts`, `src/experts/scores.ts`, `src/experts/scores.test.ts` (new), `src/cli.ts`, `CHANGELOG.md`, `AGENTS.md`, `README.md`, `.env.example`.
**Ref:** uncommitted.

## 2026-09-18 — The yes/no rulings are pluggable, and every completion claim keeps its page
**Decided:** `src/brain/judge.ts` — a `Judge` interface (verify a goal, score flow checkpoints, re-read assertions the substring match missed) and `loadJudge()`, which dynamic-imports whatever `LEAKDOWN_JUDGE` names and returns null when it is unset, missing or broken. Every ruling it cannot make returns null and the caller takes the brain path, so no judge can end a session or invent a verdict. One judge is built per session, like a brain, which is what keeps `--parallel` safe (invariant 16); whatever it wants recorded lands in `meta.json` as `usageJudge`, beside `usage`. Separately, every completion claim now appends to `verifications.jsonl` in the session dir: the page that was judged, the verdict, any `--expect` results, including claims that spend no call at all (assertion failed, verification budget used up).
**Why:** the snapshot was the one input no artifact held. A rejected completion could be read ("verification said X") but never checked, and judging a judge against past runs meant re-visiting the site, by which time the page had moved on. Proved end to end: a live run wrote one line per claim, and a replay read that file back with no browser. The interface exists because the ruling and the model that makes it are different decisions — the CLI should not care which one answers.
**Rejected:** naming a particular judge in this repo (the seam is what belongs here; a model that needs a key not every user has is not a public default); falling back to the brain when a judge returns null (two rulings per claim is two bills and two failure modes; the inconclusive path already keeps the session alive); a judge on `decide()`, persona generation or the expert panel (those produce prose, which is the product — a yes/no model has nothing to say there); per-checkpoint calls for flow scoring (one request answers them all).
**Files:** `src/brain/judge.ts` (new), `src/session.ts`, `src/site/flow.ts`, `src/site/flow.test.ts`, `src/cli.ts`, `CHANGELOG.md`, `AGENTS.md`, `README.md`.
**Ref:** uncommitted (0.8.0).
